...
/datum/controller/gameticker/proc/pregame()
	...
	spawn (0)
		...
		do
			...
			world << "<b><span style = 'notice'>Bem vindo ao pre-lobby!</span></b>"
			world << "A partida vai começar em [pregame_timeleft] segundos."
			...
			while (current_state == GAME_STATE_PREGAME)
				...
				if (pregame_timeleft == 20)
					if (tip)
						world << "<span class = 'notice'><b>Dica do Round:</b> [tip]</span>"
					else
						var/list/tips = file2list("config/tips.txt")
						if (tips.len)
							world << "<span class = 'notice'><b>Dica do Round:</b> [pick(tips)]</span>"
							qdel_list(tips)
					maytip = FALSE
...
/datum/controller/gameticker/proc/setup()
	...
	if (!map || !map.can_start() && !admin_started)
		world << "<b>Não é possivel começar a partida</b> Players inuficientes, É preciso de [map.required_players] Players. Revertendo para o Lobby."
...
/datum/controller/gameticker/proc/process()
	...
	if (finished)
		...
		spawn(50)
			...
			if (!delay_end)
				world << "<span class='notice'><big>Recomeçando em <b>90</b> segundos. Proximo Mapa: <b>TBD</b></big></span>"
