...
/obj/structure/window/examine(mob/user)
	...
	if (istype(src, /obj/structure/window/barrier))
		return
	if (health == maxhealth)
		user << "<span class='notice'>Parece intacto</span>"
	else
		var/perc = health / maxhealth
		if (perc > 0.75)
			user << "<span class='notice'>Tem algumas rachaduras</span>"
		else if (perc > 0.5)
			user << "<span class='warning'>Parece ligeiramente danificado.</span>"
		else if (perc > 0.25)
	...
	if (silicate)
		if (silicate < 30)
			user << "<span class='notice'>Possui uma fina camada de silicato.</span>"
		else if (silicate < 70)
			user << "<span class='notice'>É coberto com silicato.</span>"
		else
			user << "<span class='notice'>Há uma espessa camada de silicato cobrindo-o.</span>"
...
/obj/structure/window/proc/take_damage(var/damage = 0,  var/sound_effect = TRUE)
	...
	if (health <= 0)
		...
		if (health < maxhealth / 4 && initialhealth >= maxhealth / 4)
			visible_message("[src] parece que está prestes a se despedaçar!" )
		else if (health < maxhealth / 2 && initialhealth >= maxhealth / 2)
			visible_message("[src] parece seriamente danificado!" )
		else if (health < maxhealth * 3/4 && initialhealth >= maxhealth * 3/4)
			visible_message("As rachaduras começam a aparecer em [src]!" )
	return
...
/obj/structure/window/proc/apply_silicate(var/amount)
	if (health < maxhealth) // Mend the damage
		...
		if (health == maxhealth)
			visible_message("[src] parece totalmente consertado." )
	else // Reinforce
...
/obj/structure/window/proc/shatter(var/display_message = TRUE)
	...
	if (display_message)
		visible_message("<span class = 'warning'>[src] estilhaçado!!</span>")
...
/obj/structure/window/attack_hand(mob/user as mob)
	...
	if (usr.a_intent == I_HARM)
		...
		usr.visible_message("<span class='danger'>\[usr] bate contra \o [src]!</span>",
							"<span class='danger'>Você bate contra \o [src]!</span>",
							"Você ouve um barulho de estilhaços saindo no chão...")
	else
		playsound(loc, 'sound/effects/glassknock.ogg', 80, TRUE)
		usr.visible_message("[usr.name] batem no [name].",
							"Você bate no [name].",
...
/obj/structure/window/attack_generic(var/mob/user, var/damage)
	...
	if (!damage)
		return
	if (damage >= 10)
		visible_message("<span class='danger'>[user] se esmaga em [src]!</span>")
		take_damage(damage)
	else
		visible_message("<span class='notice'>\[user] bonks \the [src] harmlessly.</span>")
	return TRUE
...
/obj/structure/window/kick_act(var/mob/living/human/user)
	...
	visible_message("<span class='danger'>[user] chuta o [src]!</span>")
...
/obj/structure/window/attackby(obj/item/W as obj, mob/user as mob)
	...
	if (istype(W, /obj/item/weapon/grab) && get_dist(src,user)<2)
		...
		if (istype(G.affecting,/mob/living))
			...
			switch (state)
				if (1)
					M.visible_message("<span class='warning'>[user] bate [M] contra \a [src]!</span>")
					M.apply_damage(7)
					hit(10)
				if (2)
					M.visible_message("<span class='danger'>[user] bate [M] contra \a [src]!</span>")
					if (prob(50))
						M.Weaken(1)
					M.apply_damage(10)
					hit(25)
				if (3)
					M.visible_message("<span class='danger'><big>[user] bate [M] contra \a [src]!</big></span>")
					M.Weaken(5)
					M.apply_damage(20)
					hit(50)
			return
...
/obj/structure/window/proc/rotate_right()
	...
	if (anchored)
		usr << "Está presa ao chão, portanto não se pode girá-la!"
		return FALSE
