...
/obj/map_metadata
	...
	var/no_winner = "Nenhum dos lados capturou a base do outro."
	var/title = null
...
//faction stuff
	...
	var/mission_start_message = "Round vai começar em breve!"
	var/is_RP = FALSE
	var/mosinonly = FALSE
	var/squads = 1
	...
	var/current_win_condition = "Nenhum dos lados capturou a base do outro."
	...
	// lighting
	var/list/times_of_day = list("Início da manhã", "Manhã", "Meio Dia", "Tarde", "Noite", "Madrugada")
	...
	var/list/assign_precursors = list(
		"Industrias Rednikov" = list("verdine crystals","indigon crystals","galdonium crystals"),
	...
	var/winddirection = "Leste"
	var/windspeedvar = 1 // 0 to 4
	var/windspeed = "Uma brisa leve" // calm, light breeze, moderate breeze, strong breeze, gale
	var/winddesc = "Uma leve brisa do leste."
...
/obj/map_metadata/proc/wind()
	...
	winddirection = pick("Norte", "Sul", "Leste", "Oeste")
	...
	switch (windspeedvar)
		if (0)
			...
			winddesc = "Sem brisa"
		if (1)
			windspeed = "uma brisa leve"
			winddesc = "Uma brisa leve vinda do [winddirection]"
		if (2)
			windspeed = "uma brisa moderada"
			winddesc = "Uma brisa moderada vinda do [winddirection]"
		if (3)
			windspeed = "uma brisa forte"
			winddesc = "Uma brisa forte vinda do [winddirection]."
		if (4)
			windspeed = "Um vento forte"
			winddesc = "Um vento vindo do [winddirection]"
	if (windspeedvar != oldspeed)
		world << "<big>O vento muda derrepente. Agora é <b>[windspeed]</b>.</big>"
	if (winddirection != oldwind)
		world << "<big>O vento muda de direção. Agora ele está soprando do <b>[winddirection]</b>.</big>"
	...
	if (tips.len)
		for(var/client/C in clients)
			if(C.is_preference_enabled(/datum/client_preference/show_tips))
				...
				C << "<font color='#5194BB'><b>Dica:</b> [pick(tips)]</font>"
				C << "<font color='#5194BB'>---</font>"
		...
		if (!age1_done)
			...
			for(var/i = 1, i <= custom_faction_nr.len, i++)
				...
				if (count > age1_lim && world.time > 36000)
					world << "<big>O mundo vai para a era do bronze!</big>"
					age = "313 A.C."
					set_ordinal_age()
					age1_done = TRUE
			...
			for(var/i = 1, i <= custom_faction_nr.len, i++)
				...
				if (count > age2_lim && world.time >= age2_timer)
					world << "<big>O mundo avançou para a Idade Medieval!</big>"
					age = "1013"
					set_ordinal_age()
					age2_done = TRUE
			...
			for(var/i = 1, i <= custom_faction_nr.len, i++)
				...
				if (count > age3_lim && world.time >= age3_timer)
					world << "<big>O mundo avançou para a Era Imperial!</big>"
					age = "1713"
					set_ordinal_age()
					age3_done = TRUE
					if (!map.chad_mode && !map.chad_mode_plus)
			...
			for(var/i = 1, i <= custom_faction_nr.len, i++)
				...
				if (count > age4_lim && world.time >= age4_timer)
					world << "<big>O mundo avançou para a Era Industrial!</big>"
					age = "1873"
					set_ordinal_age()
					age4_done = TRUE
					if (!map.chad_mode && !map.chad_mode_plus)
			...
			for(var/i = 1, i <= custom_faction_nr.len, i++)
				...
				if (count > age5_lim && world.time >= age5_timer)
					world << "<big>O mundo avançou para a Idade Moderna!</big>"
					age = "1903"
					set_ordinal_age()
					age5_done = TRUE
					if (!map.chad_mode && !map.chad_mode_plus)
			...
			for(var/i = 1, i <= custom_faction_nr.len, i++)
				...
				if (count > age6_lim && world.time >= age6_timer)
					world << "<big>O mundo avançou para a Segunda Guerra Mundial!</big>"
					age = "1943"
					set_ordinal_age()
					age6_done = TRUE
					if (!map.chad_mode && !map.chad_mode_plus)
			...
			for(var/i = 1, i <= custom_faction_nr.len, i++)
				...
				if (count > age7_lim && world.time >= age7_timer)
					world << "<big>O mundo avançou para a Guerra Fria!</big>"
					age = "1969"
					set_ordinal_age()
					age7_done = TRUE
					if (!map.chad_mode && !map.chad_mode_plus)
			...
			for(var/i = 1, i <= custom_faction_nr.len, i++)
				...
				if (count > age8_lim && world.time >= age8_timer)
					world << "<big>O mundo avançou para a Era Moderna!</big>"
					age = "2013"
					set_ordinal_age()
					age8_done = TRUE
					if (!map.chad_mode && !map.chad_mode_plus)
...
/obj/map_metadata/proc/cross_message(faction)
	return "<font size = 4>[faction_const2name(faction,ordinal_age)] agora pode passar da parede invisivel!</font>"

/obj/map_metadata/proc/reverse_cross_message(faction)
	return "<span class = 'userdanger'>[faction_const2name(faction,ordinal_age)] Não pode mais avançar!</span>"
...
/obj/map_metadata/proc/update_win_condition()
	...
	if (world.time >= next_win && next_win != -1)
		...
		if (!map.civilizations)
			message = "[battle_name ? battle_name : "battle"] acabou em um impasse!"
		else
			message = "O round terminou!"
		if (current_winner && current_loser)
			message = "A batalha acabou! [current_winner] Ganhou de [current_loser][battle_name ? " in the [battle_name]" : ""]!"
	...
	if (!nomads && !is_singlefaction && map.ID != MAP_NOMADS_PERSISTENCE_BETA)
		...
		if (win_condition.check(typesof(roundend_condition_sides[roundend_condition_sides[2]]), roundend_condition_sides[1], roundend_condition_sides[2], 1.33, TRUE))
			if (!win_condition.check(typesof(roundend_condition_sides[roundend_condition_sides[1]]), roundend_condition_sides[2], roundend_condition_sides[1], 1.33))
				if (last_win_condition != win_condition.hash)
					current_win_condition = "[roundend_condition_def2army(roundend_condition_sides[1][1])] capturou a base de [roundend_condition_def2name(roundend_condition_sides[2][1])] Eles vão vencer em {time} minutos{s}."
		...
		// Soviet major
			...
			if (!win_condition.check(typesof(roundend_condition_sides[roundend_condition_sides[2]]), roundend_condition_sides[1], roundend_condition_sides[2], 1.33))
				if (last_win_condition != win_condition.hash)
					current_win_condition = "[roundend_condition_def2army(roundend_condition_sides[2][1])] Capturou a base de [roundend_condition_def2name(roundend_condition_sides[1][1])]! Eles vão vencer em {time} minutos{s}."
		...
		// Soviet minor
			...
			if (!win_condition.check(typesof(roundend_condition_sides[roundend_condition_sides[2]]), roundend_condition_sides[1], roundend_condition_sides[2], 1.01))
				if (last_win_condition != win_condition.hash)
					current_win_condition = "[roundend_condition_def2army(roundend_condition_sides[2][1])] have captured the [roundend_condition_def2name(roundend_condition_sides[1][1])] base! They will win in {time} minute{s}."
...
/obj/map_metadata/proc/roundend_condition_def2army(define)
	switch (define)
		...
		if (VIETNAMESE)
			return "Vietcongs"
		if (CHINESE)
			return "Exército de Libertação do Povo"
		if (FILIPINO)
			return "Exército Revolucionário Filipino"
		if (POLISH)
			return "Exército Polonês"
/obj/map_metadata/proc/army2name(army)
	switch (army)
		if ("Império Britanico")
			return "Britanico"
		if ("Pirate crew")
			return "Pirate"
		...
		if ("Império Francês")
			return "Francês"
		if ("Império Português")
			return "Português"
		if ("Império Espanhol")
			return "Espanhol"
		if ("República Holandesa")
			return "Holandês"
		if ("República Romana")
			return "Romano"
		if ("Exército Imperial")
			return "Empire"
		if ("Estados Gregos")
			return "Grego"
		if ("Arabic Caliphate")
		...
		if ("Vietcongs")
			return "Vietnamita"
		if ("Exército de Libertação do Povo")
			return "Chinês"
		if ("Exército Revolucionário Filipino")
			return "Filipino"
		if ("Exército Polonês")
			return "Polonês"
...
/obj/map_metadata/proc/seasons(var/looping = TRUE, var/force = FALSE)
	if (force || config.seasons_on)
		if (season == "FALL")
			season = "WINTER"
			world << "<big>O <b>Inverno</b> começou. Nos climas quentes, a estação das chuvas já começou.</big>"
			...
			season = "SUMMER"
			world << "<big>O <b>Verão</b> começou. Nos climas quentes, a estação seca já começou.</big>"
			change_weather_somehow()
			spawn(300)
				for (var/obj/structure/wild/tree/live_tree/TREES in world)
					TREES.change_season()
			...
			season = "SPRING"
			world << "<big>O tempo está ficando mais quente. Começou a <b>Primavera</b>. Nos climas quentes, a estação das chuvas continua.</big>"
			spawn(900)
				for (var/obj/structure/wild/tree/live_tree/TREES in world)
					TREES.change_season()
			...
			season = "FALL"
			world << "<big>As folhas começam a cair e o tempo fica mais frio. Agora é<b>Outono</b>. Nos climas quentes, a estação seca continua.</big>"
			spawn(900)
				for (var/obj/structure/wild/tree/live_tree/TREES in world)
					TREES.change_season()
