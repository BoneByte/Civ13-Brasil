...
/obj/item/weapon/horn/rojao
	name = "Rojao"
	desc = "Usado para avisar invasão"
	icon = 'caveirinha_modular/icons/itens.dmi'
	icon_state = "rojao"
	flags = CONDUCT
	slot_flags = SLOT_BELT
	...
	var/cooldown_rojao = FALSE
/obj/item/weapon/horn/rojao/attack_self(mob/user as mob)
	if (cooldown_rojao == FALSE)
		playsound(loc, 'caveirinha_modular/rojao.ogg', 100, FALSE, 25)
		user.visible_message("<h1><span class='warning'> OLHA O ROJAO!![user] SOLTOU O [name]!</span></h1>")
		cooldown_rojao = TRUE
		spawn(100)
			cooldown_rojao = FALSE
		return
