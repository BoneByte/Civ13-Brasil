...
/mob/living/simple_animal/hostile/human/Life()
	...
	if (role == "medic" && !action_running)
		help_patient()
	do_behaviour()
/mob/living/simple_animal/hostile/human/hear_say(var/message, var/verb = "fala", var/datum/language/s_language = null, var/alt_name = "",var/italics = FALSE, var/mob/speaker = null, var/sound/speech_sound, var/sound_vol, var/alt_message = null, var/animal = FALSE, var/original_message = "")
	if (stat == DEAD)
		return
	if(!isliving(speaker))
		return
