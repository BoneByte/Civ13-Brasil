...
/mob/proc/show_message(msg, type, alt, alt_type)//Message, type of message (1 or 2), alternative message, alt message type (1 or 2)
	...
	if (stat == UNCONSCIOUS || sleeping > 0)
		src << "<I>... Você quase pode ouvir alguém falando ...</I>"
	else
		src << msg
	return
...
/mob/verb/examinate(atom/A as mob|obj|turf in view())
	...
	if ((is_blind(src) || stat) && !isobserver(src))
		src << "<span class='notice'>Algo está lá, mas você não consegue ver.</span>"
		return TRUE
	...
	if (ishuman(src))
		...
		if (H.football && H.shoes && istype(H.shoes, /obj/item/clothing/shoes/football)) //if we have the ball, pass it to nearest friendly player
			...
			if (NEAR)
				...
				visible_message("[H] passa [FB] para [NEAR].")
				return
			...
			if (istype(A, /area/caribbean/football/blue/goalkeeper) || istype(A, /area/caribbean/football/red/goalkeeper))
				for(var/obj/item/football/FB in range(1,H))
					if ((!FB.owner || FB.owner == H || H.football == FB) && isturf(FB.loc))
						...
						H.football = null
						FB.owner = null
						visible_message("<font color='yellow'>[H] pega a bola!</font>")
						return
			...
			for (var/mob/living/human/HM in range(1,H))
				if (HM.civilization != H.civilization) //no tackling on same team
					if (prob(60))
						...
						visible_message("<font color='yellow'>[src] tenta lidar com [HM] mas falha!</font>")
...
/mob/proc/update_flavor_text()
	...
	if (usr != src)
		usr << "No."
	var/msg = sanitize(input(usr,"Defina o flavor text em seu verbo 'examinar'. Também pode ser usado para notas OOC sobre seu caráter.","Flavor Text",html_decode(flavor_text)) as message|null, extra = FALSE)
...
/mob/verb/abandon_mob()
	...
	if (!( config.abandon_allowed ))
		usr << "<span class='notice'>Respawn está desativado.</span>"
		return

	if ((stat != DEAD || !( ticker )))
		usr << "<span class='notice'><b>Você precisa tar morto pra usar isso, cuzão!</b></span>"
		return

	src << browse(null, "window=memory")

	src << "Você respawnar agora!"
	stop_ambience(usr)

	log_game("[name]/[key] used abandon mob.")

	usr << "<span class='notice'><b>Certifique-se de interpretar um personagem diferente e, por favor, faça RP corretamente!</b></span>"
...
/mob/verb/observe()
	...
	if (client.holder && (client.holder.rights & R_ADMIN))
		...
		usr << "<span class = 'notice'>Você precisa estar observando para usar isso!</span>"
		return
...
/mob/proc/start_pulling(var/atom/movable/AM)
	...
	if (AM.anchored || istype(AM, /obj/item/football))
		src << "<span class='warning'>Não se meche!</span>"
		return
...
/mob/Stat()
	...
	if (.)
		if (client.status_tabs && statpanel("Status") && ticker)
			...
			stat("")
			stat("Players Online (Jogando, Observando, No Lobby):", "[clients.len] ([human_clients_mob_list.len], [clients.len-human_clients_mob_list.len-new_player_mob_list.len], [new_player_mob_list.len])")
			stat("Duração do Round:", roundduration2text_days())
				...
				for (var/faction in map.faction_organization)
					...
					if (!map.civilizations)
						if (map.last_crossing_block_status[faction])
							grace_period_string += "[faction_const2name(faction,map.ordinal_age)] pode passar"
						else
							grace_period_string += "[faction_const2name(faction,map.ordinal_age)] não pode passar"
					else
						if (map.last_crossing_block_status[faction])
							grace_period_string += "A grace wall foi removida."
						else
							grace_period_string += "A grace wall está em efeito"

				stat("Grace Period Status:", grace_period_string)
				stat("Condição Round End:", map.current_stat_message())
			if (map)
				...
				switch(map.gamemode)
					...
					if ("Competitive")
						gmd = "<font color='yellow'>Competitivo</font>"
					if ("Hardcore")
						gmd = "<font color='red'>Hardcore</font>"
				stat("Mapa:", map.title)
				stat("Modo:", gmd)
				stat("Epoca:", map.age)
				stat("Estação:", get_season())
				stat("Vento:", map.winddesc)
//				stat("Weather:", get_weather())
				stat("Tempo do dia:", time_of_day)
...
mob/proc/yank_out_object()
	...
	set desc = "Remover um item embutido ao custo de sangramento e dor."
	set src in view(1)
	...
	if (usr.stat == TRUE)
		usr << "Você está inconsciente e não pode fazer isso!"
		return

	if (usr.restrained())
		usr << "Voc~e está algemado e não pode fazer isso!"
		return
	...
	if (!valid_objects.len)
		if (self)
			src << "Você não tem nada preso em seu corpo que seja suficientemente grande para ser removido."
		else
			U << "[src] não tem nada suficientemente grande para ser removido."
		return

	var/obj/item/weapon/selection = input("O que você quer retirar?", "Objetos embutidos") in valid_objects

	if (self)
		src << "<span class='warning'>Você tenta ter um bom controle sobre [selection] no seu corpo.</span>"
	else
		U << "<span class='warning'>Você tenta ter um bom controle sobre [selection] no corpo de [S]</span>"
	...
	if (self)
		visible_message("<span class='warning'><b>[src] tira [selection] do corpo dele</b></span>","<span class='warning'><b>Você tira [selection] do seu corpo.</b></span>")
	else
		visible_message("<span class='warning'><b>[usr] tira [selection] do corpo de [src].</b></span>","<span class='warning'><b>[usr] tira [selection] do seu corpo.</b></span>")
	valid_objects = get_visible_implants(0)
	...
	if (ishuman(src))
		...
		if (prob(selection.w_class * 5)) //I'M SO ANEMIC I COULD JUST -DIE-.
			...
			affected.wounds += I
			H.custom_pain("Alguma coisa rasga molhadamente em seu [affected] o [selection] é retirado brutalmente!", 30)
...
/mob/verb/face_direction()
	...
	if (!facing_dir)
		usr << "Você não tá olhando pra nenhuma direção."
	else
		usr << "Você tá olhando diretamente para [dir2text(facing_dir)]."
	if (ishuman(src))
		var/mob/living/human/H = src
