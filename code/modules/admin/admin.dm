...
/datum/admins/proc/set_custom_age()
	...
	if (!map.civilizations && !map.nomads && map.ID != MAP_TRIBES && map.ID != MAP_THREE_TRIBES && map.ID != MAP_FOUR_KINGDOMS)
		...
		var/customage = WWinput(src, "Choose the starting age:", "Starting Age", "5000 A.C.", list("5000 A.C.", "313 A.C.", "1013", "1713", "1873", "1903","1943","1969","2013", "Cancel"))
		if (customage == "Cancel")
			return
		else if (customage == "5000 A.C.")
			map.ordinal_age = 0
			map.age = "5000 A.C."
			world << "<big>The Epoch has been changed to <b>[map.age]</b>.</big>"
			log_admin("[key_name(usr)] changed the map's epoch to [map.age].")
			return
		else if (customage == "313 A.C.")
			map.ordinal_age = 1
			map.age = "313 A.C."
			map.age1_done = TRUE
